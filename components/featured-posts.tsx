...
// Sample blog posts for initial loading state
...
export function FeaturedPosts() {
  ...
  useEffect(() => {
    async function fetchFeaturedPosts() {
      try {
        const response = await fetch('/api/blog?featured=3');
        if (response.ok) {
    ...
    <section className="container mx-auto py-20 bg-muted/5">
      ...
      {loading ? (
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-72 animate-pulse rounded-lg bg-muted/20"></div>
          ))}
        </div>
      ) : (
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-2 lg:grid-cols-3">
